...
export const SpecialCharacterPalette: React.FC = () => {
    ...
    if (!isOpen) {
        return (
            <button
                ...
                className="fixed top-1/2 right-0 -translate-y-1/2 bg-[#007D8C] text-white p-2 rounded-l-md shadow-lg z-50"
    ...
    return (
        <div className="fixed top-1/2 right-0 -translate-y-1/2 bg-white p-4 rounded-l-lg shadow-xl z-50 border-l-2 border-t-2 border-b-2 border-gray-200 w-64">
            <div className="flex justify-between items-center mb-2">
                <h3 className="font-bold text-gray-700">Special Characters</h3>
                <button
                ...
                {characters.map(char => (
                    <div key={char} className="relative">
                        <button
                            ...
                            className="w-8 h-8 flex items-center justify-center text-lg bg-gray-100 hover:bg-gray-200 rounded-md transition-colors"
                            title={`Copy character: ${char}`}
                        >
                            {char}
                        </button>
                        {copiedChar === char && (
                            <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs font-bold px-2 py-1 rounded-md shadow-lg animate-fade-out">
                                Copied!
                            </div>
                        )}
                    </div>
